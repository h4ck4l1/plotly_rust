...
const MUSHROOM_GILL_ATTACHMENT_MARKDOWN: &str = include_str!("../mushroom_markdowns/mushroom_gill_attachment_markdown.md");
const MUSHROOM_GILL_ATTACHMENT_IMAGE: Asset = asset!("src/mushroom/mushroom_assets/gill_attchment.png");

#[component]
pub fn MushroomGillAttachmentColumn() -> Element {
    let mut is_hidden = use_signal(|| true);
    let mut is_plot_mounted = use_signal(|| false);
    let mut is_loaded = use_signal(|| false);
    let plot_div_id = use_signal(|| "mushroom-gill-attachment-plot");
    let table_div_id = use_signal(|| "mushroom-gill-attachment-table");
    ...
    use_effect(move || {
        ...
        spawn(async move {
            match single_col_histogram_request("gill_attachment",0f32).await {
                Ok((plot, trows)) => {
                    is_hidden.set(false);
    ...
    rsx! {
        TitleHeading { text: "Mushroom Gill Attachment Plot" }
        div {  
            ...
            div {
                ...
                img {
                    ...
                    src: MUSHROOM_GILL_ATTACHMENT_IMAGE
                }
            }
        }
        if is_hidden() {
        ...
        div {  
            ...
            div {  
                ...
                h1 {  
                    "Mushroom Gill Attachment Observations"
                }
                MarkdownComponent { text: MUSHROOM_GILL_ATTACHMENT_MARKDOWN}
            }
        }
        div {
